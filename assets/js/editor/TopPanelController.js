...
.controller('TopPanelController', ['$rootScope', '$scope', '$mdDialog', '$mdToast', '$$rAF', 'canvas', 'history', 'saver', function($rootScope, $scope, $mdDialog, $mdToast, $$rAF, canvas, history, saver) {
    ...
    $scope.openSampleImage = function() {
        $.ajax({
            type: 'GET',
            url: 'http://localhost:8888/VirtualPhoto/auth/getData',
            data: { get_param: 'value' },
            success: function (data) {
                var parsedJSON = JSON.parse(data);
                var gambar = parsedJSON.image;
                var url = 'http://localhost:8888/VirtualPhoto/assets/img/' + gambar;
    ...
    $scope.saveImage = function($event) {
        //save image sama post
        saver.saveImage($scope.imageType, $scope.imageQuality, 'Awatara', $event);
    };
